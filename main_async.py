...
from modules_fairyland.schedular_async import TaskManager
...
def main():
    ...
    if config['steps']['query']:
        scheduler = TaskManager(config = config, root_path = CURRENT_FILE_DIR, param_dt = param_dt, module = module)
        scheduler.run()
