...
,B2 as (
    select dt, sum(itemcount * gold) as sum_acquired_gold_1
    from
    (   
        select
            dt, itemid, itemcount
...
,B3 as (
    select dt, sum(itemcount * gold) as sum_acquired_gold_2
    from
    (   
        select
            dt, itemid, itemcount
...
,B4 as (
    select dt, sum(itemcount * gold) as sum_acquired_gold_3
    from
    (   
        select
            dt, itemid, itemcount
...
,B5 as (
    select dt, sum(itemcount * gold) as sum_acquired_gold_4
    from
    (   
        select
            dt, itemid, itemcount
...
,B6 as (
    select t1.dt, sum(t2.valuemax * t3.gold) as sum_acquired_gold_5
...
,C as (
    ...
    (
        ...
        union all
        select dt, sum_acquired_gold_1 from B2
        union all
        select dt, sum_acquired_gold_2 from B3
        union all
        select dt, sum_acquired_gold_3 from B4
        union all
        select dt, sum_acquired_gold_4 from B5
        union all
        select dt, sum_acquired_gold_5 from B6
    ) t
    group by dt
)
,C1 as (
...
,F1 as (
    ...
    ,lag(C.sum_gain_gold + if(C1.sum_buff_gold is not null, C1.sum_buff_gold, 0) - A.sum_consumed_gold) over(order by D.dt) as last_sum_net_gold
