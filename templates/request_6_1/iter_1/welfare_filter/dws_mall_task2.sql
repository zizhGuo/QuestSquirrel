...
select
    ...
    ,func_name
    ,sub_func_name
    ,sum(if(uid is not null, 1, 0)) as n_non_welfare_exchange_orders
    ,sum(if(uid is not null, costcount, 0)) as non_welfare_exchange_cost
    ,sum(if(uid is not null, 1, 0)) / sum(sum(if(uid is not null, 1, 0))) over(partition by dt) as non_welfare_exchange_orders_ratio
    ,sum(if(uid is not null, costcount, 0)) / sum(sum(if(uid is not null, costcount, 0))) over(partition by dt) as non_welfare_exchange_cost_ratio
from
    A
group by
    dt
...
select
    ...
    ,CONCAT(FORMAT_NUMBER(CAST(non_welfare_exchange_orders_ratio - lag(non_welfare_exchange_orders_ratio) over(partition by func_name, sub_func_name order by dt) AS DECIMAL(17, 15))*100, 2), '%') as non_welfare_exchange_orders_ratio_diff
    ,CONCAT(FORMAT_NUMBER(CAST(non_welfare_exchange_cost_ratio - lag(non_welfare_exchange_cost_ratio) over(partition by func_name, sub_func_name order by dt) AS DECIMAL(17, 15))*100, 2), '%') as non_welfare_exchange_cost_ratio_diff
from
    B
)
select * from C 
