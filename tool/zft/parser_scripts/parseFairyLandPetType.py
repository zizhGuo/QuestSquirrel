...
config_file = '_config_fairy_land_pet_type.xlsx'
sheet_name = 'Sheet1'
MY_TABLE_NAME = 'guozizhun.config_fairy_land_pet_type'
sql_file1 = '_config_fairy_land_pet_type_1_drop.sql'
sql_file2 = '_config_fairy_land_pet_type_2_create.sql'
sql_file3 = '_config_fairy_land_pet_type_3_insert.sql'
...
"""
    ...
    df = df.iloc[:, columns]
    # re index the row
    df.columns = ['level','name', 'type', 'stage', 'isbreak']
    # drop rows if id is non numeric
    df = df[df['type'].apply(lambda x: str(x).isnumeric())]
    df = df.reset_index(drop=True)
    # print(df)

    # Prepare SQL statements
    sql_statements = []
    sql += f"INSERT INTO {MY_TABLE_NAME} (level, name, type, stage, isbreak) VALUES\n"
    for index, row in df.iterrows():
        level, name, type, stage, isbreak = row
        if index != 0:
            # Format the SQL insert statement
            sql += f",({level}, '{name}', {type}, {stage}, {isbreak})\n"
        else:
            sql += f"({level}, '{name}', {type}, {stage}, {isbreak})\n"
...
columns = [0, 1, 2, 3, 4]  # Column indices for 'id', 'name', and 'functionId1'
