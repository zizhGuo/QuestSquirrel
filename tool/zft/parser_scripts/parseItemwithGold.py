...
sheet_name = 'config_item_gold'
columns = ['itemId','desc', 'gold', 'clearValue']
MY_TABLE_NAME = 'guozizhun.config_item_gold'
sql_file1 = 'config_item_gold_1_drop.sql'
sql_file2 = 'config_item_gold_2_create.sql'
sql_file3 = 'config_item_gold_3_insert.sql'
...
"""
    ...
    sql += f"INSERT INTO {MY_TABLE_NAME} (itemid, desc, gold, clearvalue) VALUES\n"
    for index, row in df.iterrows():
        itemid, desc, gold, clearvalue = row
        if index != 0:
            # Format the SQL insert statement
            sql += f",({itemid}, '{desc}', {gold}, {clearvalue})\n"
        else:
            sql += f"({itemid}, '{desc}', {gold}, {clearvalue})\n"
