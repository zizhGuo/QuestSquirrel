...
config_file = 'config_shop.xlsx'
sheet_name = 'config_rechargePackage'
columns = ['id','name', 'stampsPrice']
MY_TABLE_NAME = 'guozizhun.config_shop_price_zft'
sql_file1 = 'config_shop_price_zft_1_drop.sql'
sql_file2 = 'config_shop_price_zft_2_create.sql'
sql_file3 = 'config_shop_price_zft_3_insert.sql'
...
"""
    ...
    df = df[df['stampsPrice'].apply(lambda x: str(x).isnumeric())]
    df = df.reset_index(drop=True)
    df = df[columns]

    sql += f"INSERT INTO {MY_TABLE_NAME} (goodsid, goodsname, stamprice) VALUES\n"
    for index, row in df.iterrows():
        goodsid, goodsname, stamprice = row
