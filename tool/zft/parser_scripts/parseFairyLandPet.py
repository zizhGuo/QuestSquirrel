...
config_file = 'config_fairy_land.xlsx'
columns = ['level', 'name', 'type', 'isBreak']  # Column indices for 'id', 'name', and 'functionId1'
sheet_name = 'config_fairy_pet'
MY_TABLE_NAME = 'guozizhun.config_fairy_land_pet'
sql_file1 = 'config_fairy_land_pet_1_drop.sql'
sql_file2 = 'config_fairy_land_pet_2_create.sql'
sql_file3 = 'config_fairy_land_pet_3_insert.sql'
...
"""
    ...
    df = df.iloc[index+1:, :]
    df = df[df['level'].apply(lambda x: str(x).isnumeric())]
    df = df.reset_index(drop=True)
    df = df[columns]

    sql += f"INSERT INTO {MY_TABLE_NAME} (level, level_name, type, isbreak) VALUES\n"
    for index, row in df.iterrows():
        level, name, type, isBreak = row
        # if level == '1':
        #     continue
        isBreak = 0 if pd.isna(isBreak) else 1
        if index != 0:
            # Format the SQL insert statement
            sql += f",({level}, '{name}', {type}, {isBreak})\n"
        else:
            sql += f"({level}, '{name}', {type}, {isBreak})\n"
