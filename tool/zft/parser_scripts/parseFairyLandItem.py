...
config_file = 'config_item.xlsx'
columns = ['itemId', 'itemType', 'name', 'typeName']  # Column indices for 'id', 'name', and 'functionId1'
sheet_name = 'config_item'
MY_TABLE_NAME = 'guozizhun.config_fairy_land_item'
sql_file1 = 'config_fairy_land_item_1_drop.sql'
sql_file2 = 'config_fairy_land_item_2_create.sql'
sql_file3 = 'config_fairy_land_item_3_insert.sql'
...
"""
    ...
    sql += f"INSERT INTO {MY_TABLE_NAME} (itemid, itemtype, name, typename) VALUES\n"
    for index, row in df.iterrows():
        itemid, itemtype, name, typename = row
        # if level == '1':
        #     continue
        if index != 0:
            # Format the SQL insert statement
            sql += f",({itemid}, {itemtype}, '{name}', '{typename}')\n"
        else:
            sql += f"({itemid}, {itemtype}, '{name}', '{typename}')\n"
