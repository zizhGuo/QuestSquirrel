...
MY_TABLE_NAME = 'guozizhun.config_fish_special_red_jackpot_3d'
import pandas as pd
...
"""
    ...
    sql_statements = []
    sql += f"INSERT INTO {MY_TABLE_NAME} (id, itemid, valueMin, valueMax) VALUES\n"
    for index, row in df.iterrows():
        id, redReward = row
        _dic = {x.split(":")[0]:int(x.split(":")[1]) for x in str(redReward).split(",")}
        ...
        if index != 0:
            ...
            sql += f",({id}, {_dic['itemId']}, {_dic['valueMin']}, {_dic['valueMax']})\n"
        else:
            sql += f"({id}, {_dic['itemId']}, {_dic['valueMin']}, {_dic['valueMax']})\n"
...
excel_file = '{}/config_xlsx/{}/config_fishspecial.xlsx'.format(CURRENT_FILE_DIR, CUR_VERSION)
sheet_name = 'config_fishSpecial_red_jackpot'
columns = [1, 3]  # Column indices for 'id', 'name', and 'functionId1'
...
with open('{}/config_sql/{}/config_fish_special.sql'.format(CURRENT_FILE_DIR, CUR_VERSION), 'w', encoding='UTF-8') as f:
    f.write(sql_code)
