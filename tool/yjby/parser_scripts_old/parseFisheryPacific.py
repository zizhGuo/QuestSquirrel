...
MY_TABLE_NAME = 'guozizhun.config_fishery_pacific_3d'
import pandas as pd
...
"""
    ...
    sql_statements = []
    sql += f"INSERT INTO {MY_TABLE_NAME} (level, level_name, type) VALUES\n"
    for index, row in df.iterrows():
        level, level_name = row
        if index != 0:
            # Format the SQL insert statement
            sql += f",({level}, '{level_name}', 1)\n"
        else:
            sql += f"({level}, '{level_name}', 1)\n"
...
excel_file = '{}/config_xlsx/{}/config_fishery.xlsx'.format(CURRENT_FILE_DIR, CUR_VERSION)
sheet_name = 'config_fishery_pacific'
columns = [2, 3]  # Column indices for 'id', 'name', and 'functionId1'
...
with open('{}/config_sql/{}/config_fishery_pacific.sql'.format(CURRENT_FILE_DIR, CUR_VERSION), 'w', encoding='UTF-8') as f:
    f.write(sql_code)
