...
MY_TABLE_NAME = 'guozizhun.config_shop_price_yjby'
import pandas as pd
...
"""
    ...
    df = df.iloc[:, columns]
    # re index the row
    df.columns = ['goodsid','goodsname', 'stamprice_pair']
    # drop rows if id is non numeric
    df = df[df['goodsid'].apply(lambda x: str(x).isnumeric())]
    # df = df[df['stamprice'].apply(lambda x: str(x).isnumeric())]
    df = df.reset_index(drop=True)
    print(df)

    # Prepare SQL statements
    sql_statements = []
    sql += f"INSERT INTO {MY_TABLE_NAME} (goodsid, goodsname, stamprice) VALUES\n"
    for index, row in df.iterrows():
        goodsid, goodsname, stamprice_pair = row
        stamprice = str(stamprice_pair).split(':')[1]
        if index != 0:
            # Format the SQL insert statement
            sql += f",({goodsid}, '{goodsname}', {stamprice})\n"
        else:
            sql += f"({goodsid}, '{goodsname}', {stamprice})\n"
...
excel_file = '{}/config_xlsx/{}/config_shop.xlsx'.format(CURRENT_FILE_DIR, CUR_VERSION)
sheet_name = 'config_rechargePackage'
columns = [1, 3, 8]  # Column indices for 'id', 'name', and 'functionId1'
...
with open('{}/config_sql/{}/_config_shop_price_yjby.sql'.format(CURRENT_FILE_DIR, CUR_VERSION), 'w', encoding='UTF-8') as f:
    f.write(sql_code)
